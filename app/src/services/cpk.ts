...
interface CPKAddFundingAndStakeParams {
  amount: BigNumber
...
class CPKService {
  ...
  addFundingAndStake = async ({
    ...
    setTxState,
    useBaseToken,
  }: CPKAddFundingAndStakeParams): Promise<TransactionReceipt> => {
    try {
      const signer = this.provider.getSigner()
      const account = await signer.getAddress()
      ...
      logger.error(`There was an error adding and staking`, err.message)
      throw err
    }
  }
